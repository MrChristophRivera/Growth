...
def round_to_n(x, n):
    '''Rounds a number to n sig digits'''
    return round(x, -int(log10(x))+(n-1))
...
class Growth(object):
    ...
    def plot(self, save =False, path = '', logY=False, title =''):
        #Plots the intensity data. 
    
        #if it has a mean calculated then plot the mean as default.
        if hasattr(self, 'MeanIntensity'):
            self.MeanIntensity.plot(x=self.time, grid = False, logy=logY)
        else:    
    ...
    def subset(self,Value='', Level = 1 ):
        '''Subset the data based on the first level of Multindex of MetaData.
        
        Args:
            Value (string): The name of the value to subset on.
            Level (string): The level to suset on. 
        Return:
            Growth object'''
        ...
        if type(self.MetaData) ==str:
            ...
            #Old method for indexing. Now I am using the multindex method to to this.             
            #i = self.MetaData.columns.get_loc(Value)  #get the columm
            ...
            #subset the intensity. 
            NewGrowth.intensity = self.intensity[Value]    #this selects based on the data. 
            
            #subset the metadata. 
            #example g.MetaData[g.MetaData.iloc[:,1]=='Met']
            NewGrowth.MetaData = self.MetaData[self.MetaData.iloc[:, Level]==Value]   #subset using logical indexing
            NewGrowth.length = len(NewGrowth.intensity.columns)
            NewGrowth.Parameters = self.Parameters.loc[Value]
            
            #average the intensities
            NewGrowth.AverageIntensities()
            return NewGrowth
    ...
    def PlotParameters(self ,  Log =False, x='Concentration'):
        ...
        #Scatter for hte Delta 0D600
        ax[0].scatter(x, self.Parameters['Delta OD600'])
        ax[0].set_xlabel('Concentration', size = 14 )
        ax[0].set_ylabel('Delta OD600', size =14 )
        
        #Scatter for the  Lag times
        ax[1].scatter(x,self.Parameters['Lag Times (s)'])
        ax[1].set_xlabel('Concentration', size = 14 )
        ax[1].set_ylabel('Lag Times (s)', size =14 )
        
        #Scatter for growth rate
        ax[2].scatter(x, self.Parameters['Max Growth Rates'])
        ax[2].set_xlabel('Concentration', size =14 )
        ax[2].set_ylabel('Max Growth Rate', size =14)
        ax[2].set_ylim(-0.001, 0.01)
        
        #set up a tight layout. 
        fig.tight_layout()
   
    def setUpMultiIndex(self):
        '''This sets up a multiIndex using the metadata'''
    ...
    def AverageIntensities(self):
        ...
        self.MeanIntensity = self.intensity.groupby(axis=1, level = self.intensity.columns.names[:-1]).apply(mean, axis = 1)
